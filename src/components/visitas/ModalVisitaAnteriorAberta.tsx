...
function ModalVisitaAnteriorAberta({
  ...
  console.log(visitaAberta.entrada);
  const loading = false;

  const entrada = visitaAberta?.entrada ? dayjs(visitaAberta.entrada) : dayjs();

  const [saida, setSaida] = useState(
    entrada.add(10, "minute").format("YYYY-MM-DDTHH:mm:ss"),
  );
    ...
    >
      <div className="flex w-full flex-col gap-5 p-5">
        ...
        <Alert color="warning" icon={MdWarningAmber}>
          <span className="font-bold">Visita Anterior Aberta!</span>
          <br />
          <span className="font-medium">
            Finalize a visita e deixe uma mensagem.
          </span>
        </Alert>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            id="saida"
            type="datetime-local"
            value={saida || dayjs().format("YYYY-MM-DDTHH:mm:ss")}
